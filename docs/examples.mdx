...
- [DOM surfaces](/protocol/commands#dom): a floating slide picker via `command:dom:create-floating-element` and `command:dom:remove-element`, `command:dom:create-action-button`, `command:dom:element-rect`, `command:dom:window-size`, and fullscreen presenting with `command:dom:enter-fullscreen`, `command:dom:exit-fullscreen`, and [`subscription:dom:fullscreen-changed`](/protocol/subscriptions#dom-events).
