...
Drivers are **not** sandboxed from one another inside the worker. They share a thread, so in principle one driver can observe or interfere with another. We close that gap the same way editors like VS Code do: **every driver is reviewed before it can be published**, and you install drivers the way you trust any extension — from a source that vouches for them. It's a trust model, not a sandbox, and it's the right trade: the shared worker is what keeps drivers fast, lets them cooperate on the same scene, and keeps the protocol small. Reach for a webview whenever you want isolation on top of that trust.
