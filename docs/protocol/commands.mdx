...
- `create-floating-element`'s `position` is a fixed position in the DOM. When `barrierDismissible` is true, clicking outside dismisses it (same effect as `dom:remove-element`); the driver is **not** notified — poll `dom:element-rect` if you need to know. Defaults to false. `asPopover` defaults to true (a collision-aware popover near `position`); set it to false to render at exactly `position` as a click-through overlay — clicks pass to the canvas — for a cursor-following decoration.
- `move-element` repositions an existing floating element by `domId` — cheaper than remove + recreate, e.g. an overlay that follows the cursor. `moved` is false when no element has that `domId`.
