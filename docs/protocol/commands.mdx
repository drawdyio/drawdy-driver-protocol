...
To put pixels back on the canvas, add an `image` element whose source is a `blob` instead of a `url` (see [`DrawdyElementSchema`](/protocol/data-types#drawdyelementschema)) with `command:scene:add-drawdy-elements`. Blob-sourced images go through Drawdy's own image pipeline (local store, upload on cloud boards, one undo step, selected on insert) rather than the plain batch add, so two things differ from other elements: the element gets a Drawdy-minted id (your `drawdyElementId` is ignored), and style fields don't apply. `width`/`height` set the displayed size; `x`/`y` its top-left. Blob-sourced images can't be preview elements.
