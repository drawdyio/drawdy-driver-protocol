...
`element-clicked` fires when the named element is clicked; `element-hovered` fires when the pointer moves onto it — both are scoped to `req.domElementId` and report the pointer position in client pixels. `element-hovered` fires once per entry (when the hovered element changes), not on every pointer move within it.
