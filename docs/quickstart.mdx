...
See the full command and event catalogue in the **[Protocol Reference](/protocol)**.
...
The protocol repo ships runnable examples under [`examples/`](https://github.com/drawdyio/drawdy-driver-protocol) — `drawdy-hello`, `drawdy-math-symbols`, and `drawdy-physics-engine` — each using this same template. They are the best reference for real drivers.
