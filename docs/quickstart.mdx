...
<Note>
  A cloud board never shows these commands — the extension host would have to trust an arbitrary local server, so hot reload is restricted to local boards you own.
</Note>
...
<Note>
  Give each extension a distinct `driverId` in its `manifest.json`. Two dev servers serving the same `driverId` install over each other — Drawdy routes by `driverId`, so the id is what keeps them separate.
</Note>
